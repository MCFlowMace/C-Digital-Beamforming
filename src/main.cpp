...
int main(int argc, char **argv)
{
    ...
    //Antenna<float> ant(snr, sample_rate, wmix, 5.0f, 0.0f);

    //auto samples = ant.sample_data(1000, 0.0f, e);
    ...
    //~ for(int i=0; i<samples.frequency_data.n_elem; ++i) {
        //~ std::complex<float> x = samples.frequency_data[i];
        //~ std::cout << samples.frequency[i] << " " << x.real() << " " << x.imag() << std::endl;
    //~ }

    //~ auto data_ = samples.frequency_data.subvec(3,10);
    //~ data_.print();
