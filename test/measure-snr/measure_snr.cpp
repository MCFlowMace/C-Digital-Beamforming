...
int main(int argc, char **argv)
{
    ...
    if(argc!=2) {
        std::cerr << "args: [grid_size] !" << std::endl;
        exit(0);
    }
    ...
    for(int n_samples=n_min; n_samples<n_max; n_samples+=n_skip) {
        ...
        float amp_max = rec.get_max_val(index_max);
        ...
        float amp_noise = rec.get_mean_val(index_max);
        float snr = amp_max*amp_max/(amp_noise*amp_noise);
